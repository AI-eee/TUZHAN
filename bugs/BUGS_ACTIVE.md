...
**描述**: bleach 白名单允许 `<img>` 标签及任意 `src` 属性，用户可发送包含外部图片 URL 的消息进行追踪像素攻击。
...
**需要确认**: 合理的最大长度是多少？建议消息内容 50000 字符，身份设定 10000 字符。
...
**描述**: `username → emp_id` 迁移中，无法匹配的消息永久孤立且无日志记录。

**改进建议**: 为无法迁移的消息记录警告日志。

---

### BUG-38: 发送消息表单的接收人字段依赖 JS 填充，JS 失败时可提交空接收人
